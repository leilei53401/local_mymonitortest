 ...
 */
@Service
public class MonitorSendServiceImpl extends Thread {
    // logger
    private Logger logger = LoggerFactory.getLogger(MonitorSendServiceImpl.class);
    // 读数据的阻塞队列
...
//                            int code = 0;
    ...
    public void putLogToQueue(AdSendInfo adinfo) {
        try {
            ...
            logger.error("put adUrl To queue exception,data=" + adinfo.getCompany() + adinfo.getParams(), e);
        }
    }
}
